...
data$lifeExp[data$lifeExp == 999999] = 69.96
library("dplyr")
head(subsetdata)
subsetdata <- data %>%
  filter(country %in% c("Canada","Australia","United States"))
ggplot(data=subsetdata,aes(x=lifeExp,y=gdpPercap,group=country))+
  geom_line()+
  geom_point()
